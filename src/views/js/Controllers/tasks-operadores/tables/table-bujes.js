...
const rutaAbosluta = path.join(__dirname, 'pages-control-bujes-operador-ver.html')
...
let searchfinalizada = document.getElementById('searchfinalizada')
...
let tablesearch = document.getElementById('tableprocesosearch')
...
let mybujes;
let mybujesfinalizados;
let mybujessearch;
let mybujesfinalizadossearch

document.addEventListener('DOMContentLoaded', ()=>{
    mybujes = document.getElementById('mybujes')
    mybujessearch = document.getElementById('mybujessearch')
    mybujesfinalizados = document.getElementById('mybujesfinalizados')
    mybujesfinalizadossearch = document.getElementById('mybujesfinalizadossearch')

  getArticle()
 })

async function getArticle(){await socket.emit('get:tablecontrolbujes')}

socket.on('get:tablecontrolbujes', (results)=>{
    ...
    resultado.forEach((producto,index)=>{
        ...
        <td class='text-primary'><strong>${producto.codigoBuje}</strong></td>
          <td>${producto.nombreOperador +' '+ producto.apellidoOperador}</td>
          <td>${producto.aliasOperarios}</td>
          <td>${producto.referenciaCompuesto}</td>
...
function handleSeeButton(e){
 ...
 window.location.pathname = rutaAbosluta
}
...
socket.on('get:tablecontrolbujes', (results)=>{
  ...
  inputsearch.addEventListener('input',()=>{
    ...
    const datosfiltrados = resultado.filter((dato)=>{
      return dato.nombreOperador.toLowerCase().includes(inputDatos) || dato.apellidoOperador.toLowerCase().includes(inputDatos) || dato.aliasOperarios.toLowerCase().includes(inputDatos) || dato.referenciaCompuesto.toLowerCase().includes(inputDatos) || dato.codigoCB.toLowerCase().includes(inputDatos)
    })
  let template = ''
...
mybujessearch.innerHTML = template;
...
let finalizada = document.getElementById('tablefinalizadasearch')
let comun = document.querySelector('.finalizadas')
let searfinal = document.getElementById('searchfinalizada')

searfinal.addEventListener('input',()=>{
  if(searfinal.value !== ''){
    finalizada.classList.remove('d-none')
    comun.classList.add('d-none')
  }else{
   finalizada.classList.add('d-none')
   comun.classList.remove('d-none')
...
socket.on('get:tablecontrolbujes', (results)=>{
  ...
  resultado.forEach((producto,index)=>{
    template+= `<tr> 
    <td><i class="fab fa-angular fa-lg text-danger me-2"></i> <strong>${index +1}</strong></td>
    <td>${producto.codigoBuje}</td>
      <td>${producto.nombreOperador + ' ' + producto.apellidoOperador}</td>
      <td>${producto.aliasOperarios}</td>
      <td>${producto.referenciaCompuesto}</td>
      <td>${producto.cantidadAsignada}</td>
      <td>${(producto.revisadoPor == null) ? '<span class="rounded badge bg-label-info me-1">Pendiente</span>' : producto.revisadoPor}</td>        
      <td>${(producto.fechaRevision == null)? 'N/R' : moment(producto.fechaRevision).utc().format('DD/MM/YYYY')}</td>
...
mybujesfinalizados.innerHTML = template;
...
socket.on('get:tablecontrolbujes', (results)=>{
  let resultado = results.filter(item=>item.finalizada == 1)
  searchfinalizada.addEventListener('input',()=>{
    let inputDatos = searchfinalizada.value.toLowerCase();
    const datosfiltrados = resultado.filter((dato)=>{
      return dato.nombreOperador.toLowerCase().includes(inputDatos) || dato.apellidoOperador.toLowerCase().includes(inputDatos) || dato.aliasOperarios.toLowerCase().includes(inputDatos) || dato.referenciaCompuesto.toLowerCase().includes(inputDatos) || dato.codigoCB.toLowerCase().includes(inputDatos)
    })
  let template = ''
...
mybujesfinalizadossearch.innerHTML = template;
