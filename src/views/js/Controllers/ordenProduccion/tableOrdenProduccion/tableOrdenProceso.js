...
socket.on('getordenesproduccionstatus', (results)=>{
    let template ="";
    let ordenProduccionList = results.filter(orden=> orden.status == 'En proceso' && orden.tipo == 1);
    ...
    <tr> 
     ...
     <td>${element.cif}</td>
       ...
       <td><span class="badge rounded-pill bg-warning p-2">${element.status}</span></td>
 ...
 `;})
  ...
  inputSearch.addEventListener('input',()=>{
    ...
    let datosnuevos = ordenProduccionList.filter(filter=>{
  ...
  let template ="";
    datosnuevos.forEach((element,index)=>{
    template+=`
   <tr> 
     <td><i class="fab fa-angular fa-lg text-danger me-2"></i> <strong>${index +1}</strong></td>
