...
document.addEventListener('DOMContentLoaded', ()=>{
 myOrdenProduccion = document.getElementById('mylist')
...
socket.on('getordenesproduccionstatus', (results)=>{
    let template ="";
    let ordenProduccionList = results.filter(orden=>orden.tipo == 0);
     ordenProduccionList.reverse().forEach((element,index)=>{
      template+=`
    <tr> 
     ...
     <td>${element.cif}</td>
       ...
       <td><span class="badge rounded-pill bg-warning p-2">Especiales</span></td>
