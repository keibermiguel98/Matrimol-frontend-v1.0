...
let rutaAbsoluta = path.join(__dirname,'settings-account-usuarios.html')
...
let mysUsuarios; 
let myusuariossearch;
document.addEventListener('DOMContentLoaded', ()=>{
mysUsuarios = document.getElementById('mysUsuarios')
myusuariossearch = document.getElementById('myusuariossearch')
renderGetUsuarios()
})

async function renderGetUsuarios(){await socket.emit('get:Usuarios')}
...
inputsearch.addEventListener('input',()=>{
 ...
 socket.on('set:Usuarios',(results)=>{
   let filter = results.filter(usuario=>usuario.eliminado == 0)
    let template ="";
    ...
    <div ${element.id}>
     ...
     <td><i class="fab fa-angular fa-lg text-danger me-2"></i> <strong>${element.id}</strong></td>
       ...
       <td> ${(element.status == '0' ) ? '<span class="badge bg-label-danger me-1">Desactivado</span>' : '<span class="badge bg-label-primary me-1">Activado</span>'} </td>
       <td>
 ...
 `;
    ...
    mysUsuarios.innerHTML= template;

    btnObservar = document.querySelectorAll('.btn-ver');

    btnObservar.forEach(boton=>{ boton.addEventListener('click', renderGetUsuario)})  
      async function renderGetUsuario(e){
      const obj = parseInt(e.target.id)
      await socket.emit('idGetUsuario', obj) 
      socket.on('idUsuarioResult', (result)=>{
        const datos = JSON.stringify(result)
        window.localStorage.setItem('usuario', datos)})
        setTimeout(()=>{ window.location.pathname =  rutaAbsoluta},20)
    }
})

socket.on('set:Usuarios',(results)=>{
  let filtrado = results.filter(fil=>fil.eliminado == 0)
  inputsearch.addEventListener('input',()=>{
    let inputDatos = inputsearch.value.toLowerCase();
    const datosfiltrados = filtrado.filter((dato)=>{
      return dato.cif.toLowerCase().includes(inputDatos) || dato.nombre.toLowerCase().includes(inputDatos) || dato.apellido.toLowerCase().includes(inputDatos) || dato.rol.toLowerCase().includes(inputDatos)
    })
  let template ="";
...
`;
  ...
  myusuariossearch.innerHTML= template;
